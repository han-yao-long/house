<template>
  <div id="imgList" @click.stop>
    ...
    <div class="bg">
      ...
      <ul :class="{h70: tab == 2}">
        <li v-for="(item,index) in arr" :key="index">
          ...
          <div class="phoone">{{item.phone}}</div>
          <div class="nu">{{item.image_count}}张</div>
        </li>
      </ul>
      <div v-show="islast" class="last">
          <div class="indexs">{{paihangmsg.current_rank.raning}}</div>
          <div class="name">{{paihangmsg.current_rank.name}}</div>
          <div class="phoone">{{paihangmsg.current_rank.phone}}</div>
    ...
    <p class="dayTime">每周日晚上8点，系统更新每周排名</p>
  </div>
</template>
<script>
export default {
